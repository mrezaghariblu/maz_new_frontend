    ...
    `
      ...
      .history-item {
        ...
        .timeline-dot {
          ...
          flex-shrink: 0;
          margin-top: 4px;
          background: var(--maz-firouzeh-500);
          border: 2px solid var(--maz-firouzeh-200);
        }
        .h-info {
          flex: 1;
          .h-status {
  ...
  template: `
    ...
    @if (loading()) {
      ...
      <div class="profile-header">
        ...
        @if (auth.isSuperuser()) {
          <button
            class="maz-btn maz-btn--sm"
            [class.maz-btn--danger]="user()!.canLogin"
            [class.maz-btn--ghost]="!user()!.canLogin"
            (click)="toggleCanLogin()"
          >
            {{ user()!.canLogin ? '🔒 غیرفعال کردن ورود' : '🔓 فعال کردن ورود' }}
          </button>
      ...
      <div class="detail-grid">
        ...
        <div class="maz-card">
          <div class="maz-flex-between maz-gap-8" style="margin-bottom:16px">
            <span style="font-size:14px;font-weight:800;color:var(--maz-firouzeh-900)"
              >اطلاعات پایه</span
            >
            <maz-status-badge
          ...
          <div class="info-row">
            <span class="ir-label">نام</span
            ><span class="ir-val">{{ user()!.firstName }} {{ user()!.lastName }}</span>
          </div>
          <div class="info-row">
            <span class="ir-label">کد ملی</span
            ><span class="ir-val">{{ user()!.nationalCode }}</span>
          </div>
          <div class="info-row">
            <span class="ir-label">جنسیت</span
            ><span class="ir-val">{{ user()!.gender === 'MALE' ? 'مرد' : 'زن' }}</span>
          </div>
          <div class="info-row">
            <span class="ir-label">تلفن</span><span class="ir-val">{{ user()!.phone ?? '—' }}</span>
          </div>
          <div class="info-row">
            <span class="ir-label">ایمیل</span
            ><span class="ir-val">{{ user()!.email ?? '—' }}</span>
          </div>
          <div class="info-row">
            <span class="ir-label">لاگین</span
            ><span class="ir-val">{{ user()!.canLogin ? '✅ دارد' : '❌ ندارد' }}</span>
          </div>
          <div class="info-row">
            <span class="ir-label">تاریخ ثبت</span
            ><span class="ir-val">{{ user()!.createdAt | date: 'yyyy/MM/dd' }}</span>
          </div>
        </div>
        ...
        <div class="maz-card">
          <div
            style="font-size:14px;font-weight:800;color:var(--maz-firouzeh-900);margin-bottom:16px"
          >
            تخصیص مراکز
          </div>
          @for (a of user()!.centerAssignments; track a.id) {
            <div class="assign-item">
              ...
              <div class="a-meta">
                ...
                @if (a.revokedAt) {
                  <span style="color:var(--maz-danger)"
                    >تا {{ a.revokedAt | date: 'yyyy/MM/dd' }}</span
                  >
                } @else {
                  <span class="maz-badge maz-badge--active" style="font-size:10px;padding:1px 6px"
                    >فعال</span
                  >
                }
              </div>
              @if (a.note) {
                <div style="font-size:11px;color:var(--maz-gray-400);margin-top:3px">
                  {{ a.note }}
                </div>
              }
            </div>
          ...
          <div class="history-item">
            ...
            <div class="h-info">
              ...
              <div class="h-dates">
                ...
                @if (h.endDate) {
                  تا {{ h.endDate | date: 'yyyy/MM/dd' }}
                } @else {
                  <span style="color:var(--maz-success)">— جاری</span>
                }
    ...
    @if (showTransferModal) {
      <div class="modal-overlay">
        <div class="modal">
          ...
          <div class="modal-body">
            <div class="maz-form-group">
              <label class="maz-label"
                >مرکز مبدأ
                <span style="color:var(--maz-gray-400);font-weight:400">(اختیاری)</span></label
              >
              <select
                class="maz-select"
                [(ngModel)]="transferForm.fromCenterId"
                name="fromCenterId"
              >
            ...
            <div class="maz-form-group">
              ...
              <input
                ...
                placeholder="دلیل نقل‌وانتقال"
              />
            </div>
          </div>
          <div class="modal-foot">
            <button class="maz-btn maz-btn--ghost" (click)="showTransferModal = false">
              انصراف
            </button>
...
export class UserDetailComponent implements OnInit {
  ...
  activeAssignments = () => (this.user()?.centerAssignments ?? []).filter((a) => !a.revokedAt);

  toggleCanLogin() {
    if (!this.user()) return;
    const u = this.user()!;
    const newVal = !u.canLogin;
    const msg = newVal
      ? 'آیا می‌خواهید دسترسی ورود این کاربر را فعال کنید؟'
      : 'آیا می‌خواهید دسترسی ورود این کاربر را غیرفعال کنید؟';
    if (!confirm(msg)) return;
    this.usersApi.setCanLogin(u.id, { canLogin: newVal }).subscribe({
      next: () => {
        this.appState.toast(newVal ? 'دسترسی ورود فعال شد' : 'دسترسی ورود غیرفعال شد', 'success');
        // reload user
        this.usersApi.getOne(u.id).subscribe((updated) => this.user.set(updated));
      },
      error: (e: any) => this.appState.toast(e.error?.message ?? 'خطا', 'error'),
    });
  }
    ...
    this.usersApi
      .transfer(u.id, {
        fromCenterId: this.transferForm.fromCenterId
          ? Number(this.transferForm.fromCenterId)
          : undefined,
  ...
  DISABILITY_SEVERITY_LABEL: Record<string, string> = {
    MILD: 'خفیف',
    MODERATE: 'متوسط',
    SEVERE: 'شدید',
  };
